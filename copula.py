...
class GaussianCopula(Distribution):
    ...
    def conditional_sample(
        ...
        # m, n = cond_val.shape[0], self.event_shape[0]  # for example, taking the first element from each
        ...
        reg_coeff = torch.linalg.solve(cov_11, cov_10)  # Sigma_{11}^{-1} Sigma_{10}
        cond_mu = torch.mv(reg_coeff.t(), cond_val_nscale)
        cond_sigma = cov_00 - torch.mm(cov_01, reg_coeff)
        L = torch.linalg.cholesky(cond_sigma, upper=False)
        cond_sigma = torch.mm(L, L.t())
